...
echo Begin to add apppool %AppPoolname%
...
mkdir "%ALLUSERSPROFILE%\%Vdirname%"
call InstallUser.bat  "%ALLUSERSPROFILE%\%Vdirname%"
%appcommand% add vdir /app.name:"%SiteName%/" /path:/%Vdirroot%  /physicalPath:"%ALLUSERSPROFILE%\%Vdirname%" 
@if %errorlevel% == 0 (
