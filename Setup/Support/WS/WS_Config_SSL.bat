...
if not "%AMQPPORT%" == "" if not "%TSLVERSION%" == "" (
...
cscript.exe "%CurrentDir%\replacefiletext.vbs" "%INSTALLPATH%\AmqpOption.json" "|tsl| : false" "|tsl| : true" "%INSTALLPATH%\AmqpOption.json"
cscript.exe "%CurrentDir%\replacefiletext.vbs" "%INSTALLPATH%\AmqpOption.json" "|tsl_version| : |Tls12|" "|tsl_version| : |%TSLVERSION%|" "%INSTALLPATH%\AmqpOption.json"
